...
mem_init(void)
{
  ...
  boot_map_region(kern_pgdir, UPAGES, ROUNDUP((sizeof(struct PageInfo) * npages), PGSIZE), PADDR(pages), (PTE_W | PTE_P));
	...
	// Your code goes here:
    /* Lab4 */
	boot_map_region(kern_pgdir, KSTACKTOP - KSTKSIZE, ROUNDUP(KSTKSIZE, PGSIZE), PADDR(bootstack), 
									(PTE_W | PTE_P));
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if(!(*ppde & PTE_P) && !create) // What does the front do?
		return NULL;
	else if(!(*ppde & PTE_P) && create){
		pp = page_alloc(1);
		if(pp == NULL) // fail to allocate new page
			return NULL;
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	...
	for(currSize=0; currSize < size; currSize+=PGSIZE){
		currVA = va + currSize;
		currPA = pa + currSize;
		ppte = pgdir_walk(pgdir, (void *) currVA, 1);
		
		if(ppte != NULL)
			*ppte = (PTE_ADDR(currPA) | perm | PTE_P);
	}
}
...
setupkvm()
{
	...
	boot_map_region(ppde, KSTACKTOP-KSTKSIZE, KSTKSIZE, PADDR(bootstack), PTE_W);
	boot_map_region(ppde, KERNBASE, (1<<32)-KERNBASE, 0, PTE_W);
  boot_map_region(ppde, IOPHYSMEM, ROUNDUP((EXTPHYSMEM - IOPHYSMEM), PGSIZE), IOPHYSMEM, (PTE_W));
